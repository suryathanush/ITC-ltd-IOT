...
def run():
    ...
    while(1):
        try:
            ...
            client = connect_mqtt()
            client.loop_start()
            file = open("/home/surya/Downloads/IOT-standalone/client_side/cache.csv", "r")
            file_content = file.read()
            file.close()
            #---------------Start publishing if cache file is not empty and connected to server------
            if(Connect_Status & (not file_content=="")):
                with open('/home/surya/Downloads/IOT-standalone/client_side/cache.csv', 'r') as csvfile:
                ...
                with open('/home/surya/Downloads/IOT-standalone/client_side/cache.csv', 'w') as csvfile:
                    csvfile.truncate()
            else:
                client.loop_stop()
